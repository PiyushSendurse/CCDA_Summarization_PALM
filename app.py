...
def generate_summary(data):
    ...
    prompt = f"Summarize the following data where entire data is for one patient only:\n{section_prompt}"
...
#     if "Medical_History" in data:
#         medical_history_data = data["Medical_History"]
#         lines = medical_history_data.strip().split('\n')
#         headers = lines[2].split('|')
#         section_data = [line.split('|') for line in lines[4:-1]]
#
#         print(section_data)
#
#         positive_conditions = [condition.strip() for condition, response in section_data if
#                                response.strip().lower() == "y"]
#
#         if positive_conditions:
#             section_prompt = "The individual has the following medical conditions with 'yes' responses:\n"
#             for condition in positive_conditions:
#                 section_prompt += f"- {condition}\n"
#         else:
#             section_prompt = "The individual has no medical conditions with 'yes' responses."
#     else:
#         # Use your default prompt for other sections
...
def summarize_selected_sections():
    ...
    for section_name in selected_sections:
        section_data = extract_all_table_data(section_name,root)  # Pass the 'root' element
...
def extract_medical_data():
    ...
    for section_name in sections_to_extract:
        section_data = extract_all_table_data(section_name, root)
        # print(section_data)
...
#         if section_name == 'Medical History':
#             # Special handling for 'Medical History' section
#             if section_data:
#                 summary = generate_summary(section_data)
#                 section_key = section_name.replace(" ", "_")
#                 extracted_data[section_key] = summary
...
#                         else:
#                             cell_data = "None Recorded"  # Use "None Recorded" for empty cells
...
#                 })
#
#             break  # Once the section is found and processed, exit the loop
#
#     return data
...
#             header_table_pairs = []
#             for row in table_data:
#                 # Check if header is not None before applying replace
#                 header_table_pair = {
#                     header.replace(" ", "_"): value
#                     for header, value in zip(headers, row)
#                     if header is not None
#                 }
#                 header_table_pairs.append(header_table_pair)
#
#             section_key_values[section_name.replace(" ", "_")] = header_table_pairs
#
#         extracted_data[section_name.replace(" ", "_")] = section_key_values
#
#     return jsonify(extracted_data)
@app.route("/extract_dynamic_data", methods=["GET"])
def extract_dynamic_data():
    ...
    for section_name in section_names:
        ...
        # Check if the section has any data
        if any(entry["table_data"] for entry in section_data):
            section_key_values = {}
            for entry in section_data:
                headers = entry["headers"]
                table_data = entry["table_data"]
            ...
            extracted_data[section_name.replace(" ", "_")] = [{"None_Recorded": "None Recorded"}]
...
def cleanup_uploaded_file():
    global uploaded_xml_path
    if uploaded_xml_path:
        os.remove(uploaded_xml_path)

# Register the cleanup function
atexit.register(cleanup_uploaded_file)
